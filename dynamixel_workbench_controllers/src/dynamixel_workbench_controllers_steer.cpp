   ...
   is_moving_(false)
{
  ...
  dxl_wbb_ = new DynamixelWorkbench;
...
bool DynamixelController::initWorkbench(const std::string port_name, const uint32_t baud_rate)
{
  ...
  result1 = dxl_wbb_->init(port_name.c_str(), baud_rate, &log);
  if (result1 == false)
  {
    ROS_ERROR("%s", log);
  }
...
bool DynamixelController::loadDynamixels(void)
{
  ...
  for (auto const& dxl:dynamixel_)
  {
    ...
    result1 = dxl_wbb_->ping((uint8_t)dxl.second, &model_number, &log);
    if (result1 == false)
    {
      ROS_ERROR("%s", log);
...
bool DynamixelController::initDynamixels(void)
{
  ...
  for (auto const& dxl:dynamixel_)
  {
    dxl_wbb_->torqueOff((uint8_t)dxl.second);
    ...
    for (auto const& info:dynamixel_info_)
    {
      if (dxl.first == info.first)
      {
        ...
        {
          bool result1 = dxl_wbb_->itemWrite((uint8_t)dxl.second, info.second.item_name.c_str(), info.second.value, &log);
          if (result1 == false)
          {
            ROS_ERROR("%s", log);
    ...
    dxl_wbb_->torqueOn((uint8_t)dxl.second);
  }
...
bool DynamixelController::initControlItems(void)
{
  ...
  const ControlItem *goal_position = dxl_wbb_->getItemInfo(it->second, "Goal_Position");
  if (goal_position == NULL) return false;

  const ControlItem *goal_velocity = dxl_wbb_->getItemInfo(it->second, "Goal_Velocity");
  if (goal_velocity == NULL)  goal_velocity = dxl_wbb_->getItemInfo(it->second, "Moving_Speed");
  if (goal_velocity == NULL)  return false;

  const ControlItem *present_position = dxl_wbb_->getItemInfo(it->second, "Present_Position");
  if (present_position == NULL) return false;

  const ControlItem *present_velocity = dxl_wbb_->getItemInfo(it->second, "Present_Velocity");
  if (present_velocity == NULL)  present_velocity = dxl_wbb_->getItemInfo(it->second, "Present_Speed");
  if (present_velocity == NULL) return false;

  const ControlItem *present_current = dxl_wbb_->getItemInfo(it->second, "Present_Current");
  if (present_current == NULL)  present_current = dxl_wbb_->getItemInfo(it->second, "Present_Load");
...
bool DynamixelController::initSDKHandlers(void)
{
  ...
  result1 = dxl_wbb_->addSyncWriteHandler(control_items_["Goal_Position"]->address, control_items_["Goal_Position"]->data_length, &log);
  if (result1 == false)
  {
    ROS_ERROR("%s", log);
    return result1;
  ...
  result1 = dxl_wbb_->addSyncWriteHandler(control_items_["Goal_Velocity"]->address, control_items_["Goal_Velocity"]->data_length, &log);
  if (result1 == false)
  {
    ROS_ERROR("%s", log);
    return result1;
  ...
  if (dxl_wbb_->getProtocolVersion() == 2.0f)
  {  
    ...
    */    
    ...
    result1 = dxl_wbb_->addSyncReadHandler(start_address,
                                          read_length,
                                          &log);
    if (result1 == false)
    {
...
bool DynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  ...
  if (dxl_wbb_->getProtocolVersion() == 2.0f)
  {
    result1 = dxl_wbb_->syncRead(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                               id_array,
                               dxl_name.size(),
                               &log);
    if (result1 == false)
    ...
    result1 = dxl_wbb_->getSyncReadData(SYNC_READ_HANDLER_FOR_PRESENT_POSITION_VELOCITY_CURRENT,
                                      id_array,
                                      id_cnt,
    ...
    else
    {
      ...
      {
        wp.position = dxl_wbb_->convertValue2Radian(id_array[index], get_position[index]);
        pre_goal_.push_back(wp);
      }
    }
  }
  else if (dxl_wbb_->getProtocolVersion() == 1.0f)
  {
    WayPoint wp;
    uint32_t read_position;
    for (auto const& dxl:dynamixel_)
    {
      result1 = dxl_wbb_->readRegister((uint8_t)dxl.second,
                                     control_items_["Present_Position"]->address,
                                     control_items_["Present_Position"]->data_length,
                                     &read_position,
                                     &log);
      ...
      wp.position = dxl_wbb_->convertValue2Radian((uint8_t)dxl.second, read_position);
      pre_goal_.push_back(wp);
    }
  }
...
void DynamixelController::publishCallback(const ros::TimerEvent&)
{
  ...
  if (is_joint_state_topic_)
  {
    ...
    for (auto const& dxl:dynamixel_)
    {
      ...
      if (dxl_wbb_->getProtocolVersion() == 2.0f)
      {
        if (strcmp(dxl_wbb_->getModelName((uint8_t)dxl.second), "XL-320") == 0) effort = dxl_wbb_->convertValue2Load((int16_t)dynamixel_state_list_.dynamixel_state[id_cnt].present_current);
        else  effort = dxl_wbb_->convertValue2Current((int16_t)dynamixel_state_list_.dynamixel_state[id_cnt].present_current);
      }
      else if (dxl_wbb_->getProtocolVersion() == 1.0f) effort = dxl_wbb_->convertValue2Load((int16_t)dynamixel_state_list_.dynamixel_state[id_cnt].present_current);

      velocity = dxl_wbb_->convertValue2Velocity((uint8_t)dxl.second, (int32_t)dynamixel_state_list_.dynamixel_state[id_cnt].present_velocity);
      position = dxl_wbb_->convertValue2Radian((uint8_t)dxl.second, (int32_t)dynamixel_state_list_.dynamixel_state[id_cnt].present_position);
...
void DynamixelController::commandVelocityCallback(const geometry_msgs::Twist::ConstPtr &msg)
{
  ...
  for (auto const& dxl:dynamixel_)
  {
    const ModelInfo *modelInfo = dxl_wbb_->getModelInfo((uint8_t)dxl.second);
    rpm = modelInfo->rpm;
    id_array[id_cnt++] = (uint8_t)dxl.second;
  }
...
//       = r * ((RPM * Goal_Velocity) * 0.10472)		=> Goal_Velocity = V / (r * RPM * 0.10472) = V * VELOCITY_CONSTATNE_VALUE
  ...
  if (dxl_wbb_->getProtocolVersion() == 2.0f)
  {
    if (strcmp(dxl_wbb_->getModelName(id_array[0]), "XL-320") == 0)
    {/*
    ...
    else
    {
      ...
      dynamixel_position[0] = dxl_wbb_->convertRadian2Value(id_array[0], wheel_angle[0]);
      dynamixel_position[1] = dxl_wbb_->convertRadian2Value(id_array[1], wheel_angle[1]);
      dynamixel_position[2] = dxl_wbb_->convertRadian2Value(id_array[2], wheel_angle[2]);
      dynamixel_position[3] = dxl_wbb_->convertRadian2Value(id_array[3], wheel_angle[3]);
      
      /*for (int i = 0; i<=3; i++)
      {
        if (wheel_angle[i]>=3.14)
        {
          dynamixel_position[i] = 4095;
        }
      ...
      dynamixel_velocity[2]  = -wheel_velocity[2] * velocity_constant_value;
      dynamixel_velocity[3] = wheel_velocity[3] * velocity_constant_value;*/
      std::cout << "ID5 wheel angle = " << wheel_angle[0] << "ID5 dynamixel_position= " << dynamixel_position[0] << std::endl;
      std::cout << "ID6 wheel angle = " << wheel_angle[1] << "ID6 dynamixel_position= " << dynamixel_position[1] << std::endl;
      std::cout << "ID7 wheel angle = " << wheel_angle[2] << "ID7 dynamixel_position= " << dynamixel_position[2] << std::endl;
      std::cout << "ID8 wheel angle = " << wheel_angle[3] << "ID8 dynamixel_position= " << dynamixel_position[3] << std::endl;
  ...
  result1 = dxl_wbb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array, dynamixel_.size(), dynamixel_position, 1, &log);
  if (result1 == false)
  {
    ROS_ERROR("%s", log);
  }
...
void DynamixelController::writeCallback(const ros::TimerEvent&)
{
  ...
  {
    for (uint8_t index = 0; index < id_cnt; index++)
      dynamixel_position[index] = dxl_wbb_->convertRadian2Value(id_array[index], jnt_tra_msg_->points[point_cnt].positions.at(index));

    result1 = dxl_wbb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array, id_cnt, dynamixel_position, 1, &log);
    if (result1 == false)
    {
      ROS_ERROR("%s", log);
    }
...
void DynamixelController::trajectoryMsgCallback(const trajectory_msgs::JointTrajectory::ConstPtr &msg)
{
  ...
  if (is_moving_ == false)
  {
    ...
    result1 = getPresentPosition(msg->joint_names);
    if (result1 == false)
                                                      ...
                                                      dynamixel_workbench_msgs::DynamixelCommand::Response &res)
{
  ...
  result1 = dxl_wbb_->itemWrite(id, item_name.c_str(), value, &log);
  if (result1 == false)
  {
    ROS_ERROR("%s", log);
...
int main(int argc, char **argv)
{
  ...
  std::string port_name = "/dev/steer";
  ...
  result1 = dynamixel_controller.initWorkbench(port_name, baud_rate);
  if (result1 == false)
  {
  ...
  result1 = dynamixel_controller.getDynamixelsInfo(yaml_file);
  if (result1 == false)
  {
  ...
  result1 = dynamixel_controller.loadDynamixels();
  if (result1 == false)
  {
  ...
  result1 = dynamixel_controller.initDynamixels();
  if (result1 == false)
  {
  ...
  result1 = dynamixel_controller.initControlItems();
  if (result1 == false)
  {
  ...
  result1 = dynamixel_controller.initSDKHandlers();
  if (result1 == false)
  {
